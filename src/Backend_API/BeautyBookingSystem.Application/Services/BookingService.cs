...
namespace BeautyBookingSystem.Application.Services
{
    ...
    //======================================== đoạn này mới thêm xác thực sdt nếu chưa nhé ===
        ...
        public async Task<bool> RescheduleBookingAsync(int customerId, int bookingId, RescheduleBookingRequest request)
        {
            ...
            if (booking.Status != BookingStatus.Pending && booking.Status != BookingStatus.Confirmed && booking.Status != BookingStatus.DepositPaid)
        ...
        public async Task<List<AvailableStaffDto>> GetAvailableStaffAsync(GetAvailableStaffRequest request)
        {
            ...
            return activeStaffs
                .Where(s =>
                    ...
                    s.Schedules.Any(sch =>
                    MatchDayOfWeek(sch.DayOfWeek, appointmentDayOfWeek) &&
                    sch.IsWorking &&
        ...
        public async Task<List<BookingTimeSlotDto>> GetAvailableTimeSlotsAsync(GetAvailableTimeSlotsRequest request)
        {
            if (request.ServiceIds == null || !request.ServiceIds.Any())
                throw new InvalidOperationException("ServiceIds không được rỗng");
            ...
            while (currentTime.Add(TimeSpan.FromMinutes(totalDurationMinutes)) <= closingTime)
            {
                ...
                bool isPastSlot = request.Date.Date == nowVN.Date && currentTime <= nowVN.TimeOfDay;
                bool isAvailable = false;

                if (!isPastSlot)
                {
                    var availableStaffs = await GetAvailableStaffAsync(new GetAvailableStaffRequest
                    {
                        StoreId = request.StoreId,
        ...
        public async Task<BookingPolicyDto> GetBookingPoliciesAsync(int customerId)
        {
            int bookingMinHours = await _systemConfigService.GetValueAsync<int>(SystemConfigKeys.BookingMinHours);
            if (bookingMinHours <= 0) bookingMinHours = 1;

            int cancelBeforeHours = await _systemConfigService.GetValueAsync<int>(SystemConfigKeys.CancelBeforeHours);
            if (cancelBeforeHours <= 0) cancelBeforeHours = 2;

            int gracePeriodMinutes = await _systemConfigService.GetValueAsync<int>(SystemConfigKeys.GracePeriodMinutes);
            if (gracePeriodMinutes <= 0) gracePeriodMinutes = 15;

            int maxCancelPerDay = await _systemConfigService.GetValueAsync<int>(SystemConfigKeys.MaxCancelPerDay);
            if (maxCancelPerDay < 0) maxCancelPerDay = 5;
            ...
            int rescheduleBeforeHours = await _systemConfigService.GetValueAsync<int>(SystemConfigKeys.RescheduleBeforeHours);
            if (rescheduleBeforeHours <= 0) rescheduleBeforeHours = 2;

            var nowVN = DateTime.UtcNow.AddHours(7);
            var todayStart = nowVN.Date;
            var todayEnd = todayStart.AddDays(1);

            int currentCancelCountToday = await _unitOfWork.BookingRepository.GetQueryable()
                .Where(b => b.CustomerId == customerId
                            && b.Status == BookingStatus.Cancelled
                            && b.CancelledBy == CancelledByType.Customer
                            && b.UpdatedAt >= todayStart
                            && b.UpdatedAt < todayEnd)
                .CountAsync();

            int currentNoShowCount = await CountCustomerNoShowsAsync(customerId);

            bool isBookingBlocked = blockUserIfNoShow && currentNoShowCount >= noShowLimit;

            string? blockReason = null;
            if (isBookingBlocked)
            {
                blockReason = $"Tài khoản bị khóa đặt lịch do đã có {currentNoShowCount}/{noShowLimit} lần no-show.";
            }
            ...
            return new BookingPolicyDto
            {
                ...
                MaxCancelPerDay = maxCancelPerDay,
                NoShowLimit = noShowLimit,
                BlockUserIfNoShow = blockUserIfNoShow,
                RescheduleBeforeHours = rescheduleBeforeHours,
                IsBookingBlocked = isBookingBlocked,
                CurrentNoShowCount = currentNoShowCount,
                CurrentCancelCountToday = currentCancelCountToday,
        ...
        private static bool MatchDayOfWeek(DayOfWeek storedDayOfWeek, DayOfWeek requestedDayOfWeek)
        {
            var stored = (int)storedDayOfWeek;
            var requested = (int)requestedDayOfWeek;
        ...
        private static DayOfWeek[] GetDayOfWeekCandidates(DayOfWeek dateDayOfWeek)
        {
            return dateDayOfWeek == DayOfWeek.Sunday
                ? new[] { DayOfWeek.Sunday, (DayOfWeek)7 }
                : new[] { dateDayOfWeek };
        }
    }
}