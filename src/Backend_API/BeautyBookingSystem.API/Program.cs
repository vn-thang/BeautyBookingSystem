...
builder.Services.AddScoped<ICustomerFavoriteService, CustomerFavoriteService>();
builder.Services.AddScoped<ISearchHistoryService, SearchHistoryService>();
builder.Services.AddScoped<IReviewService, ReviewService>(); // Đã sửa lỗi thiếu Interface
builder.Services.AddScoped<ICustomerUserService, CustomerUserService>();
builder.Services.AddScoped<IPublicStoreService, PublicStoreService>();
...
// ===== FIREBASE INIT (ĐÃ SỬA LỖI) =====
var firebaseKeyPath = Path.Combine(Directory.GetCurrentDirectory(), "firebase-key.json"); 
if (File.Exists(firebaseKeyPath))
{
    if (FirebaseApp.DefaultInstance == null) // Đảm bảo không bị khởi tạo đè
    {
        FirebaseApp.Create(new AppOptions()
        {
            Credential = GoogleCredential.FromFile(firebaseKeyPath)
...
// ===== CORS CONFIG (ĐÃ SỬA LỖI) =====
builder.Services.AddCors(options =>
{
    options.AddPolicy("GlobalCorsPolicy", policy =>
    {
        policy.WithOrigins("http://localhost:5173") // ReactJS Admin
              .AllowAnyHeader()
              .AllowAnyMethod()
              .AllowCredentials() // Cho phép gửi Token/Cookie
              .SetIsOriginAllowed(origin => true); // Cho phép mọi App Mobile/Frontend khác gọi vào
    });
});
...
// Chỉ dùng 1 Policy duy nhất để tránh xung đột
